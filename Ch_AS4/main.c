...
volatile int TimeEvent_1ms = NO;		// pAction_Fn() �����̸� �ֱ�����...
...
#define CNT_LWIP_PERIODIC_HANDLE			1		// ù��°
...
uint32_t remoteStartInCnt = 0;		// 100ms ����
uint32_t remoteStartInTime = 0;		// 100ms ����

uint32_t remoteStartOutSig = NO;	// ����Ʈ ���ڷ� ����ؾ� �ȴ�.
uint32_t remoteStartOutCnt = 0;
uint32_t remoteStartOutTime = 1;		// 100ms ����
...
#if RS422_ENABLE
#if RS422_MASTER
// (rxv_intr_ext) RS422���� ���ͷ�Ʈ �߻�		 -   My_ID �̰ų� Global_ID
			if(rs422if.rx_intr == 1) {
				rs422if.rx_intr = 0;
				if(devState.connected == NO) rs422_PacketParse_Master();
			}
#else
			rs422_SendData();	// Slave --> Master    /** ���ź��� ���ʿ� ��ġ�ϴ� ���� ...*/
			if(rs422if.rx_intr == 1) {
				rs422if.rx_intr = 0;
				...
				case CNT_STATE_CHECK_BASE:
					if(++stateCheckCnt >= CNT_STATE_CHECK) {	// 2Hz
						...
						if(devState.adReadValid) {
							CheckDeviceState();		// Power Button ���� , Door , Magnetic switch , leak
						}
					}
					break;
